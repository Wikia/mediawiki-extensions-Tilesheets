...
class TileList extends SpecialPage {
	...
	public function execute( $par ) {
		...
		$conditions = [ "entry_id >= $from" ];
		if ( $formattedEntryIDs != '' ) {
			$conditions[] =
				'entry_id ' . ( $opts->getValue( 'invertlang' ) == 1 ? 'NOT' : '' ) . ' IN (' .
				$formattedEntryIDs . ')';
		}
		if ( $mod != '' ) {
		...
		try {
			...
			if ( $searchNames ) {
				$conditions =
					array_replace(
						$conditions,
						[ count( $conditions ) - 1 => "item_name = {$dbr->addQuotes($regex)}" ]
					);
			}
			$result = $dbr->select(
				'ext_tilesheet_items',
		...
		$canTranslate =
			$this->permissionManager->userHasRight( $this->getUser(), 'translatetiles' );
		$table .= "!";
		if ( $canEdit ) {
			$table .= " !!";
		}
		...
		$linkStyle =
			"style=\"width:23px; padding-left:5px; padding-right: 5px; text-align:center; font-weight:bold;\"";
		foreach ( $results as $result ) {
			...
			if ( $canEdit ) {
				$editLink =
					"[[Special:TileManager/$lId|" . $this->msg( 'tilesheet-edit' )->text() . "]]";
			...
			$translateLink =
				$canTranslate ? "[[Special:TileTranslator/$lId|" .
								$this->msg( 'tilesheet-tile-list-translate' )->text() . "]]" : '';

			$viewLink =
				"[[Special:ViewTile/$lId|" . $this->msg( 'tilesheet-tile-list-view' ) . "]]";
		...
		if ( $page == 0 ) {
			...
			if ( $page == 1 ) {
				$prevPage =
					"[{{fullurl:{{FULLPAGENAME}}|regex=" . $opts->getValue( 'regex' ) . "&mod=" .
					$opts->getValue( 'mod' ) . "&langs=" . $opts->getValue( 'langs' ) .
					"&invertlang=" . $opts->getValue( 'invertlang' ) . "&from=" .
					$opts->getValue( 'from' ) . "&limit=" . $opts->getValue( 'limit' ) . "}} " .
					$this->msg( 'tilesheet-pagination-first-arrow' )->text() . "]";
			} else {
				$prevPage =
					"[{{fullurl:{{FULLPAGENAME}}|regex=" . $opts->getValue( 'regex' ) . "&mod=" .
					$opts->getValue( 'mod' ) . "&langs=" . $opts->getValue( 'langs' ) .
					...
					$opts->getValue( 'from' ) . "&limit=" . $opts->getValue( 'limit' ) . "}} " .
					$this->msg( 'tilesheet-pagination-first-arrow' )->text() .
					"] [{{fullurl:{{FULLPAGENAME}}|page={$pPage}&mod=" . $opts->getValue( 'mod' ) .
					"&langs=" . $opts->getValue( 'langs' ) . "&invertlang=" .
					$opts->getValue( 'invertlang' ) . "&limit=" . $opts->getValue( 'limit' ) .
					"}} " . $this->msg( 'tilesheet-pagination-prev' )->text() . "]";
			}
		}
		if ( $lPage == $page ) {
			...
			if ( $lPage == $page + 1 ) {
				$nextPage =
					"[{{fullurl:{{FULLPAGENAME}}|page={$nPage}&regex=" .
					$opts->getValue( 'regex' ) . "&mod=" . $opts->getValue( 'mod' ) . "&langs=" .
					$opts->getValue( 'langs' ) . "&invertlang=" . $opts->getValue( 'invertlang' ) .
					"&from=" . $opts->getValue( 'from' ) . "&limit=" . $opts->getValue( 'limit' ) .
					"}} " . $this->msg( 'tilesheet-pagination-last-arrow' )->text() . "]";
			} else {
				$nextPage =
					"[{{fullurl:{{FULLPAGENAME}}|page={$nPage}&regex=" .
					$opts->getValue( 'regex' ) . "&mod=" . $opts->getValue( 'mod' ) . "&langs=" .
					$opts->getValue( 'langs' ) . "&invertlang=" . $opts->getValue( 'invertlang' ) .
					"&from=" . $opts->getValue( 'from' ) . "&limit=" . $opts->getValue( 'limit' ) .
					"}} " . $this->msg( 'tilesheet-pagination-next' )->text() .
					"] [{{fullurl:{{FULLPAGENAME}}|page={$lPage}&regex=" .
					$opts->getValue( 'regex' ) . "&mod=" . $opts->getValue( 'mod' ) . "&langs=" .
					$opts->getValue( 'langs' ) . "&invertlang=" . $opts->getValue( 'invertlang' ) .
		...
		$pageSelection =
			"<div style=\"text-align:center;\" class=\"plainlinks\">$prevPage | $nextPage</div>";

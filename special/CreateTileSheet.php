	...
	public function execute( $par ) {
		...
		if ( $_POST ) {
			...
			if ( $opts->getValue( 'update_table' ) == 1 ) {
				...
				$out->addHtml(
					$this->returnMessage(
						SheetManager::deleteEntry(
							$mod,
							$this->getUser(),
							$this->msg( 'tilesheet-create-summary-deletesheet' )->text()
						),
						$this->msg( 'tilesheet-create-response-msg-deletesheet' )->text()
					)
				);
				// Truncate table
				$out->addHtml(
					$this->returnMessage(
						SheetManager::truncateTable(
							$mod,
							$this->getUser(),
							$this->msg( 'tilesheet-create-summary-deletesheet' )->text()
						),
						$this->msg( 'tilesheet-create-response-msg-truncate' )->text()
					)
				);
			}
			...
			$out->addHtml(
				$this->returnMessage(
					SheetManager::createSheet( $mod, $sizes, $this->getUser() ),
					$this->msg( 'tilesheet-create-response-msg-newsheet' )->text()
				)
			);
				...
				$out->addHtml(
					$this->returnMessage(
						TileManager::createTile(
							...
							$z,
							$this->getUser(),
							$this->msg( 'tilesheet-create-summary-newtile' )->text()
						),
						$this->msg( 'tilesheet-create-response-msg-newtile' )->params(
							$item,
							$mod,
							$x,
							$y,
	...
	private function returnMessage( $state, $message ) {
		if ( $state ) {
			$out =
				...
				$this->msg( 'tilesheet-create-response-success' )->text() . '</span> ' . $message .
				"<br>";
		} else {
			$out =
				'<span style="background-color:red; font-weight:bold; color:white;">' .
				$this->msg( 'tilesheet-create-response-fail' )->text() . '</span> ' . $message .
				"<br>";
		}

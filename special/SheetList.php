...
class SheetList extends SpecialPage {
	...
	public function execute( $par ) {
		...
		foreach ( $results as $result ) {
			...
			if ( $canEdit ) {
				$editLink =
					" style=\"width:23px; padding-left:5px; padding-right:5px; text-align:center; font-weight:bold;\" | [[Special:SheetManager/$lMod|" .
					$this->msg( 'tilesheet-edit' )->text() . "]] ||";
			} else {
				$editLink = "";
			}
		...
		if ( $page == 0 ) {
			...
			if ( $page == 1 ) {
				$prevPage =
					"[{{fullurl:{{FULLPAGENAME}}|limit=" . $opts->getValue( 'limit' ) . "}} " .
					$this->msg( 'tilesheet-pagination-first-arrow' )->text() . "]";
			} else {
				$prevPage =
					"[{{fullurl:{{FULLPAGENAME}}|limit=" . $opts->getValue( 'limit' ) . "}} " .
					$this->msg( 'tilesheet-pagination-first-arrow' )->text() .
					"] [{{fullurl:{{FULLPAGENAME}}|page={$pPage}&limit=" .
					$opts->getValue( 'limit' ) . "}} " .
		...
		if ( $lPage == $page ) {
			...
			if ( $lPage == $page + 1 ) {
				$nextPage =
					"[{{fullurl:{{FULLPAGENAME}}|page={$nPage}&limit=" .
					$opts->getValue( 'limit' ) . "}} " .
				...
				$nextPage =
					...
					"] [{{fullurl:{{FULLPAGENAME}}|page={$lPage}&limit=" .
					$opts->getValue( 'limit' ) . "}} " .
					$this->msg( 'tilesheet-pagination-last-arrow' )->text() . "]";
			}
		}

...
class WhatUsesThisTile extends SpecialPage {
	...
	public function execute( $entryID ) {
		...
		$tileData = $dbr->select( 'ext_tilesheet_items', '*', [ 'entry_id' => $entryID ] );
		...
		if ( $tileData->numRows() == 0 ) {
			...
			$out->addWikiTextAsInterface(
				$this->msg( 'tilesheet-whatusesthistile-notile', $entryID )
			);
			...
			$out->setPageTitle(
				$this->msg(
					'tilesheet-whatusesthistile-title-full',
					$name,
					$tileData->current()->mod_name
				)
			);
			...
			if ( $results->numRows() == 0 ) {
				$out->addWikiTextAsInterface(
					$this->msg( 'tilesheet-whatusesthistile-none', $name )
				);

				return;
			} else {
				$out->addWikiTextAsInterface(
					$this->msg( 'tilesheet-whatusesthistile-some', $name )
				);
				...
				foreach ( $rows as $row ) {
					...
					$out->addHtml(
						Xml::openElement( 'li' ) .
						$this->getLinkRenderer()->makeKnownLink( $title ) .
						Xml::closeElement( 'li' )
					);
				}
	...
	private function getPrevNext( $prevID, $nextID, $opts ) {
		...
		foreach ( [ 20, 50, 100, 250, 500 ] as $limit ) {
			...
			$limitLinks[] =
				$this->makeSelfLink( $prettyLimit, array_merge( $changed, $overrides ) );
		}

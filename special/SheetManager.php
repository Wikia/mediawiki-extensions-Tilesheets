...
class SheetManager extends SpecialPage {
	...
	public function execute( $par ) {
		...
		if ( $opts->getValue( 'truncate' ) == 1 ||
			 $opts->getValue( 'delete' ) == 1 && $userIsSysop ) {
	...
	public static function deleteEntry( $mod, $user, $comment = "" ) {
		...
		$stuff = $dbw->select( 'ext_tilesheet_images', '*', [ '`mod`' => $mod ] );
		$result = $dbw->delete( 'ext_tilesheet_images', [ '`mod`' => $mod ] );
		...
		$logEntry->setParameters( [ "4::mod" => $mod, "5::sizes" => $stuff->current()->sizes ] );
	...
	public static function truncateTable( $mod, $user, $comment = "" ) {
		$dbw = MediaWikiServices::getInstance()->getDBLoadBalancer()->getConnection( DB_PRIMARY );
		$stuff = $dbw->select( 'ext_tilesheet_items', '*', [ 'mod_name' => $mod ] );
		$result = $dbw->delete( 'ext_tilesheet_items', [ 'mod_name' => $mod ] );
	...
	public static function createSheet( $mod, $sizes, $user, $comment = "" ) {
		...
		$result =
			$dbw->select( 'ext_tilesheet_images', 'COUNT(`mod`) AS count', [ '`mod`' => $mod ] );

		// Insert into tilesheet list
		if ( $result->current()->count == 0 ) {
			$dbw->insert( 'ext_tilesheet_images', [ '`mod`' => $mod, '`sizes`' => $sizes ] );
		} else {
			return false;
		}
		...
		$logEntry->setTarget( Title::newFromText( "Sheet/$mod", NS_SPECIAL ) );
		$logEntry->setComment( $comment );
		$logEntry->setParameters( [ "4::mod" => $mod, "5::sizes" => $sizes ] );
		$logId = $logEntry->insert();
		$logEntry->publish( $logId );
	...
	private function displayUpdateForm( $mod ) {
		...
		$result = $dbr->select( 'ext_tilesheet_images', '*', [ '`mod`' => $mod ] );
		if ( $result->numRows() == 0 ) {

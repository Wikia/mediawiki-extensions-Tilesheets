...
class TileTranslator extends SpecialPage {
	...
	public static function updateTable(
		...
		$stuff =
			$dbw->select( 'ext_tilesheet_languages', '*', [ 'entry_id' => $id, 'lang' => $language ]
			);
		if ( $stuff->numRows() == 0 ) {
			$dbw->insert(
		...
		$item = $dbw->select( 'ext_tilesheet_items', 'item_name', [ 'entry_id' => $id ] );
	...
	public static function deleteEntry( $id, $language, $user, $comment = "" ) {
		...
		$dbw->delete( 'ext_tilesheet_languages', [ 'entry_id' => $id, 'lang' => $language ] );
		$logEntry = new ManualLogEntry( 'tilesheet', 'deletetranslation' );
		$logEntry->setPerformer( $user );
		$logEntry->setComment( $comment );
		$logEntry->setTarget( Title::newFromText( "Tile/$id", NS_SPECIAL ) );
		$logEntry->setParameters( [ '4::id' => $id, '5::lang' => $language ] );
		$logID = $logEntry->insert();
		$logEntry->publish( $logID );
	...
	private function displayUpdateForm( $id, $language ) {
		...
		$result =
			$dbr->select( 'ext_tilesheet_languages', '*', [ 'entry_id' => $id, 'lang' => $language ]
			);
